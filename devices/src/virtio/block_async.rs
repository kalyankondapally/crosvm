...
use base::{error, info, iov_max, warn, EventFd, TimerFd};
...
use super::{
    copy_config, queue, DescriptorChain, DescriptorError, Interrupt, Queue, Reader, VirtioDevice,
    Writer, TYPE_BLOCK, VIRTIO_F_VERSION_1,
};
...
async fn process_one_request_task(
    ...
    let _notify_disable = queue::NotifyGuard::new(Rc::clone(&queue), mem.clone());
...
mod tests {
    ...
    fn read_size() {
        ...
        let b = BlockAsync::new(Box::new(f), true, false, 512, None).unwrap();
    ...
    fn read_block_size() {
        ...
        let b = BlockAsync::new(Box::new(f), true, false, 4096, None).unwrap();
    ...
    fn read_features() {
        ...
        {
            let f = File::create(&path).unwrap();
            let b = BlockAsync::new(Box::new(f), false, true, 512, None).unwrap();
        ...
        {
            let f = File::create(&path).unwrap();
            let b = BlockAsync::new(Box::new(f), false, false, 512, None).unwrap();
        ...
        {
            let f = File::create(&path).unwrap();
            let b = BlockAsync::new(Box::new(f), true, true, 512, None).unwrap();
