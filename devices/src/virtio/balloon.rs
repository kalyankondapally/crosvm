...
use base::{self, error, info, warn, EventFd};
use cros_async::{select6, U64Source};
...
async fn handle_queue<F>(
    ...
    F: FnMut(GuestAddress) -> (),
{
    let mut queue_event = match U64Source::new(queue_event) {
        Ok(e) => e,
        Err(e) => {
...
async fn handle_stats_queue(
    ...
    mut queue: Queue,
    queue_event: EventFd,
    mut stats_read_evt: U64Source<EventFd>,
...
// the U64Source will be read from the stats queue task.
...
fn setup_stat_signal() -> std::result::Result<(EventFd, U64Source<EventFd>), StatSignalError> {
    let stat_signal_event = EventFd::new().map_err(StatSignalError::Sys)?;
    let stat_read_event = stat_signal_event
        .try_clone()
        .map_err(StatSignalError::Sys)?;
    let stat_async_event = U64Source::new(stat_read_event).map_err(StatSignalError::Poll)?;
